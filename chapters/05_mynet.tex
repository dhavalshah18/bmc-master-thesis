\section{Triplanar-Net \todo{name tbd}}
\label{chapter5}
...
The input arms (and concept of using 2D MIPs) is heavily inspired by the BtrflyNet architecture, but the input portion is made deeper and an added axial view is also used \cite{sekuboyina2018}. \todo{more about BtrflyNet, why use that? how it is relevant to add third view in this context?} 
...
\todo{labels? as mips maybe? but we use 3d labels, so need to find way to show/visualize this}
...
\img{trinet.pdf}{\linewidth}{\todo{caption}}{Triplanar-Net architecture.}

\img{2d_to_3d.pdf}{0.5\linewidth}{\todo{caption}}{2D to 3D reconstruction block}
...
In comparison to the networks briefly described in Chapter \ref{chapter4}, Triplanar-Net consists of a more shallow architecture, as well as both 2D and 3D convolutions. Generally, a shallower network can lead to poorer overall results be it for any task. However, by efficiently combining two dimensional and three dimensional data and convolutions, the network is able to be lightweight. \todo{probably reiterate}. 
